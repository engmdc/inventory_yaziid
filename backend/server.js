...
app.post('/api/users', async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO users (id, username, password, role, name) VALUES (?, ?, ?, ?, ?)', [id, username, password, role, name]);
...
app.get('/api/customers', async (req, res) => {
    try {
        ...
        // Convert debt back to float correctly and map snake_case to camelCase
        res.json(rows.map(c => ({
            ...c,
            ownerId: c.owner_id,
...
app.post('/api/customers', async (req, res) => {
    try {
        ...
        // On Duplicates UPDATE (UPSERT)
        await pool.query(`
            INSERT INTO customers (id, name, phone, email, debt, notes, owner_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE 
            name = VALUES(name), phone = VALUES(phone), email = VALUES(email), debt = VALUES(debt), notes = VALUES(notes)
...
app.post('/api/products', async (req, res) => {
    try {
        ...
        await pool.query(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE 
            name = VALUES(name), category = VALUES(category), price = VALUES(price), cost = VALUES(cost), stock = VALUES(stock)
...
// Sync entire stock list (For bulk deductions in sale)
app.patch('/api/products/bulk-stock', async (req, res) => {
    const { updates } = req.body; // [{id, newStock}]
...
app.get('/api/transactions', async (req, res) => {
    try {
        ...
        // Match items securely to transactions
        const result = transactions.map(tx => {
            const txItems = items.filter(item => item.transaction_id === tx.id).map(i => ({
...
app.post('/api/transactions', async (req, res) => {
    const { id, type, amount, total, subtotal, discount, paymentMethod, customerId, customerName, customId, ownerId, items } = req.body;
    const connection = await pool.getConnection();

    try {
        await connection.beginTransaction();

        // Add transaction core
        await connection.query(`
            INSERT INTO transactions (id, type, amount, total, subtotal, discount, payment_method, customer_id, customer_name, custom_id, owner_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [id, type, amount || 0, total || 0, subtotal || 0, discount || 0, paymentMethod, customerId, customerName, customId, ownerId]);

        // Add transaction items if they exist
